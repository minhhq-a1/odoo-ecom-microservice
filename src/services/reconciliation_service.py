...
class ReconciliationService:
    ...
    async def run_for_platform(
        ...
        async with get_async_db_context() as db:
            ...
            field_drift_mappings = await self._load_drift_mappings(
                db,
                platform,
                {o.platform_order_id for o in platform_orders if o.platform_order_id in synced_ids},
            )
        ...
        drifts = await self._check_field_drift(
            platform, platform_orders, synced_ids, field_drift_mappings
        )
        ...
        if result.needs_review > 0 or result.drifts:
            await AlertService.send_reconciliation_alert(
                platform,
                result.missing_details + [d.__dict__ for d in drifts],
            )
    ...
    async def run_all_platforms(self, run_date: date | None = None) -> list[ReconciliationResult]:
        ...
        for p in platforms:
            try:
                ...
                await AlertService.send_reconciliation_alert(
                    p,
                    [{"error": str(e), "phase": "run_for_platform"}],
                )
        return results
    ...
    async def _check_field_drift(
        self,
        platform: str,
        platform_orders: list[UnifiedOrder],
        synced_ids: set[str],
        mappings: list,
    ) -> list[Drift]:
        drifts: list[Drift] = []
        synced = {
            o.platform_order_id: o for o in platform_orders if o.platform_order_id in synced_ids
        }
        ...
        for mapping in mappings:
            ...
            try:
                rows = await self.odoo.search_read(
                    "sale.order",
                    [["id", "=", mapping.odoo_order_id]],
                    ["amount_total", "state", "x_tracking_number"],
                    limit=1,
                )
            except Exception as e:
                logger.warning(
                    "drift_odoo_read_failed", odoo_id=mapping.odoo_order_id, error=str(e)
                )
                continue
            if not rows:
                continue
            ...
            if abs(pf.total_amount - odoo_amount) > DRIFT_TOTAL_TOLERANCE_VND:
                drifts.append(
                    Drift(
                        platform=platform,
                        platform_order_id=pf.platform_order_id,
                        field="total_amount",
                        platform_value=str(pf.total_amount),
                        odoo_value=str(odoo_amount),
                    )
                )
            ...
            if pf_tracking and pf_tracking != odoo_tracking:
                drifts.append(
                    Drift(
                        ...
                        platform_value=pf_tracking,
                        odoo_value=odoo_tracking,
                    )
                )
        return drifts
