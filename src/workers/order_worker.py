...
def process_webhook_event(
    ...
    logger.info("process_webhook_event_start",
                outbox_id=outbox_id, platform=platform, event_type=event_type)
        ...
        if event_type == "order" and platform == "shopee":
            if settings.MIDDLEWARE_DRY_RUN:
                logger.info(
                    ...
                    outbox_id=outbox_id, platform_order_id=platform_order_id,
                )
    ...
    try:
        ...
        logger.exception("product_not_found_dead_letter",
                     sku=e.sku, outbox_id=outbox_id)
        ...
        try:
            ...
            logger.warning(
                ...
                outbox_id=outbox_id, error=str(persist_err),
            )
        ...
        logger.warning("circuit_open_retry_scheduled",
                       outbox_id=outbox_id, countdown_sec=countdown, error=str(e))
        raise self.retry(exc=e, countdown=countdown)
    except ShopeeRateLimitError as e:
        countdown = getattr(e, "retry_after", None) or SHOPEE_RL_COUNTDOWN_FALLBACK
        logger.warning("shopee_rate_limit_retry_scheduled",
                       outbox_id=outbox_id, countdown_sec=countdown)
        raise self.retry(exc=e, countdown=countdown)
    except OdooConnectionError as e:
        logger.warning("odoo_connection_retry_scheduled",
                       outbox_id=outbox_id,
                       countdown_sec=ODOO_CONN_COUNTDOWN,
                       attempt=self.request.retries + 1, error=str(e))
...
def sync_order_to_odoo(self, order_mapping_id: int) -> dict[str, Any]:
    ...
    async def _run() -> dict[str, Any]:
        ...
        if settings.MIDDLEWARE_DRY_RUN:
            logger.info(
                ...
                order_mapping_id=order_mapping_id, platform_order_id=order_sn,
            )
            return {"status": "dry_run_skipped"}
    ...
    try:
        ...
        countdown = circuit_retry_countdown(circuit_service_from_error(e))
        logger.warning("circuit_open_retry_scheduled_manual",
                       order_mapping_id=order_mapping_id,
                       countdown_sec=countdown, error=str(e))
        raise self.retry(exc=e, countdown=countdown)
    except OdooConnectionError as e:
        logger.warning("odoo_connection_retry_scheduled_manual",
                       order_mapping_id=order_mapping_id,
